  ...
  <div class="bg-white dark:bg-gray-800 p-4 h-full flex flex-col">
    ...
    <div class="mb-4">
      <div class="flex flex-wrap border-b border-gray-200 dark:border-gray-700">
        <div v-for="(part, index) in parts" :key="part.id" class="relative mr-2">
          <button @click="activePart = `part-${index}`"
            class="inline-flex items-center py-2 px-4 text-sm font-medium text-center rounded-t-lg" :class="{
            ...
            <!-- Delete Button -->
            <span role="button" tabindex="0" @click.stop="removePart(index)" @keydown.enter.stop="removePart(index)"
    ...
    <div class="tab-content">
      <div v-for="(part, index) in parts" :key="part.id" v-show="activePart === `part-${index}`" class="pt-2">
      </div>
    </div>
    ...
    <div class="flex-1 pr-1">
      <div v-if="parts.length === 0" class="text-center py-12">
        ...
        <p class="text-sm text-gray-400 dark:text-gray-500">Add a part using the "Add Part" button in the test
          information panel.</p>
      </div>
      ...
      <div v-else class="flex flex-col sm:flex-row gap-3 mb-4">
        <button @click="addNewQuestion"
          ...
          <div
            class="w-10 h-10 flex items-center justify-center bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 rounded-lg mr-3 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/30 transition-colors">
      ...
      <div v-else class="space-y-4">
        <template v-for="(item, index) in currentItems" :key="index">
          <!-- Question Set -->
          <div v-if="item._type === 'questionSet'"
            class="border-2 border-gray-100 dark:border-gray-700 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            <div
              class="bg-gray-50 dark:bg-gray-700/80 px-4 py-3 border-b border-gray-200 dark:border-gray-600 flex justify-between items-center">
              ...
              <button @click.stop="removeQuestion(index, true)"
                class="text-gray-400 hover:text-red-500 dark:hover:text-red-400 p-1" title="Remove question set">
            ...
            <div class="p-4 bg-white dark:bg-gray-800">
              <p class="text-sm text-gray-600 dark:text-gray-300 mb-3"
                v-html="item.description || 'No description provided.'"></p>
              <div class="space-y-3">
                <QuestionListItem v-for="(question, qIndex) in item.questions || []" :key="qIndex" :question="question"
                  :question-number="question.order" :is-selected="false" :show-remove="false"
          ...
          <!-- Single Question -->
          <QuestionListItem v-else :question="item" :question-number="item.order"
            :is-selected="selectedQuestion === index" :show-remove="true" @select="selectQuestion(index)"
            @remove="removeQuestion(index)" />
        </template>

        <div v-if="currentItems.length === 0" class="text-center py-8 text-gray-500 dark:text-gray-400 text-sm">
