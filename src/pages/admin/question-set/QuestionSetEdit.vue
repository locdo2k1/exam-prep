<template>
          ...
          <div class="grid grid-rows-[auto_minmax(0,1fr)] gap-4 h-full min-h-0">
            ...
            <div class="flex flex-col min-h-0 space-y-3">
              ...
              <Editor 
                ...
                placeholder="Write your question set description here..."
              />
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    emits: ['questions-selected', 'loaded'],
    setup(props, { emit }) {
      const route = useRoute();
      const router = useRouter();
      const questionSetId = computed(() => route.params.id);
      ...
      onMounted(async () => {
        try {
          ...
          // Load question set data if ID is provided in URL
          if (questionSetId.value) {
            await loadQuestionSet();
          } else {
            console.error('No question set ID found in URL');
            toast.error('Invalid question set ID');
            router.push({ name: 'admin-question-sets' });
          }
      ...
      // Load question set data if ID is provided
      const loadQuestionSet = async () => {
        ...
        try {
          isLoading.value = true;
          const response = await questionSetApi.getById(questionSetId.value);
          
          if (response?.success && response.data) {
            const data = response.data;
            questionSetTitle.value = data.title;
            questionContent.value = data.description || '';
            
            // Set selected questions if any
            if (data.questions?.length > 0) {
              // Clear existing selections
              selectedQuestionsSet.value.clear();
              
              // Add questions to selected set (only id and prompt)
              data.questions.forEach(question => {
                selectedQuestionsSet.value.add({
                  id: question.id,
                  prompt: question.prompt
                });
              });
              
              // Update available questions to reflect selection
              if (availableQuestions.value.length > 0) {
                availableQuestions.value = availableQuestions.value.map(q => ({
                  ...q,
                  isSelected: Array.from(selectedQuestionsSet.value).some(sq => sq.id === q.id)
                }));
              }
              
              // Emit selected questions
              const selectedQuestionsArray = Array.from(selectedQuestionsSet.value);
              emit('questions-selected', selectedQuestionsArray);
              
            }
            
            emit('loaded', data);
            toast.success('Question set loaded successfully');
          } else {
            toast.error(response?.message || 'Failed to load question set data');
          }
        } catch (error) {

          toast.error(error.response?.data?.message || 'An error occurred while loading the question set');
        } finally {
          isLoading.value = false;
        }
      };
      ...
      const saveQuestionSet = async () => {
        ...
        try {
          ...
          // Show loading state
          const loadingToast = toast.info('Updating question set...', { timeout: false });
          
          let response;
          try {
            response = await questionSetApi.update(questionSetId.value, questionSetData);
          ...
          if (response.success) {
            ...
            // Show error toast
            toast.error(`Failed to update question set: ${response.message}`);
          }
        } catch (error) {
